...
function ServicesPageInner() {
  ...
  const { items, size: vehicleSize, add, remove, setSize: setVehicleSize, count, total } = useCart();
  ...
  return (
    <main>
      <Section>
        ...
        <div className="flex flex-col gap-12">
          {servicesByCategory.map(group => (
            <section key={group.category}>
              <Heading level={2} className="mb-4">{group.category}</Heading>
        ...
        <button
          className="fixed bottom-6 right-6 bg-brand text-white px-6 py-3 rounded-full shadow-lg font-medium text-base hover:bg-brand/90 transition"
