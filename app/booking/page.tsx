...
export default function BookingPage() {
  ...
  const addons = useMemo(() => (qp.get("addons") || "").split(",").filter(Boolean), [qp]);
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {pkg && <span className="inline-flex items-center rounded-full border px-3 py-1 text-sm">{pkg}</span>}
        {addons.map(a => (
