...
export default function RequestClient() {
    ...
    return (
      <div className="bg-surface rounded-lg shadow p-8 text-center mt-8">
        ...
        <a
          ...
          className="inline-block bg-brand text-white px-6 py-2 rounded font-bold hover:bg-brand-600 transition mb-2 focus-ring"
        >
          Book with Setmore
        </a>
        <br />
        <a href="/confirmation?appointmentStarted=1" className="inline-block bg-brand text-white px-6 py-2 rounded font-bold hover:bg-brand-600 transition focus-ring">Go to Confirmation</a>
      </div>
    );
  }
  ...
  return (
    <form className="grid gap-6 md:grid-cols-2" onSubmit={handleSubmit}>
      ...
      <div>
        <label className="block text-sm font-medium text-text mb-1">Photos (up to 3, JPG/PNG/WebP, max 5MB each)</label>
        <input
          ref={fileInputRef}
          type="file"
      ...
      <button type="submit" className="w-full bg-brand text-white rounded-xl px-5 py-3 font-bold hover:bg-brand-600 transition focus-ring" disabled={submitting}>
