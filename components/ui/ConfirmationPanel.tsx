...
export default function ConfirmationPanel({ booked }: ConfirmationPanelProps) {
  return (
    <div className="bg-white/90 rounded-xl shadow p-8 text-center max-w-lg mx-auto mt-12">
      ...
      {booked ? (
        <a href={process.env.NEXT_PUBLIC_SETMORE_URL || "https://sakanastudiollc.setmore.com/zechariah"} target="_blank" rel="noopener noreferrer" className="inline-block bg-accent text-charcoal px-6 py-3 rounded-xl font-bold hover:bg-accent/80 transition">View in Setmore</a>
      ) : (
