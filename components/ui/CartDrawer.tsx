  ...
  return (
    ...
    <aside className={`fixed inset-y-0 right-0 max-w-md w-full bg-surface shadow-xl z-50 transform transition-transform duration-200 ${open ? "translate-x-0" : "translate-x-full"}`} aria-label="Cart Drawer">
      <div className="flex justify-between items-center p-4 border-b">
        <h2 className="text-xl font-bold heading text-brand">Your Cart</h2>
        <button onClick={onClose} aria-label="Close cart" className="text-text hover:text-brand text-2xl focus-ring">×</button>
      </div>
      ...
      <div className="p-4 border-t border-subtle flex flex-col gap-2">
        <label className="text-sm font-medium text-text mb-1">Vehicle Size</label>
        ...
        <button onClick={onContinue} className="w-full mt-4 bg-brand text-white py-2 rounded font-bold hover:bg-brand-600 transition focus-ring">Continue</button>
      </div>
    </aside>
    </>
  );
