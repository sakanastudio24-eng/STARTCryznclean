...
export default function ServiceCard({ id, title, price, basePrice, category, selected, onClick }: ServiceCardProps) {
  return (
    <div
      ...
      <button
        className={`mt-5 inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-semibold transition-colors focus-ring ${selected ? "bg-brand text-white" : "border-2 border-subtle text-foreground hover:bg-surfaceAlt"}`}
