    ...
    <nav className="flex items-center justify-center gap-4 py-6" aria-label="Progress">
      {steps.map((label, i) => (
        <div key={label} className="flex items-center gap-2">
          <span className={`rounded-full w-8 h-8 flex items-center justify-center font-bold text-lg border-2 ${i === step ? "bg-brand text-white border-brand" : "bg-surface text-foreground border-subtle"}`}>{i + 1}</span>
