    ...
    <section className="w-full bg-accent/10 py-12 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {displayImages.map((img) => (
          <figure key={img.file} className="relative rounded-lg shadow bg-white overflow-hidden">
            <img
              src={"/images/" + img.file}
              alt={img.alt}
              loading="lazy"
            ...
            />
            <figcaption className="absolute bottom-0 left-0 right-0 p-2 text-sm text-offWhite text-center bg-charcoal/60">
              {img.caption}
            </figcaption>
          </figure>
        ))}
