...
export default function GalleryGrid({ preview = false }: { preview?: boolean }) {
  ...
  return (
    <section className="w-full">
      ...
      <h2 className="text-3xl font-bold heading text-brand mb-8 text-center">Gallery</h2>
      <div className="flex justify-center gap-2 mb-6 flex-wrap">
        {categories.map(cat => (
          <button
            key={cat}
            className={`px-4 py-1 rounded-full font-medium border transition focus-ring ${category === cat ? "bg-brand text-white border-brand" : "bg-white text-brand border-brand/30"}`}
